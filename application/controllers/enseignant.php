	...
	class Enseignant extends CI_Controller {
		...
		public function insertSeance() {
			...
			// Limiter l'access
		        ...
		        {
					// fails
	                $this->load->view('insert_Seance', $data);
		        }
				else
				{
		...
		public function listeAbs() {
			...
			if($this->session->userdata('type') == 'enseignant') {
				...
				if (!$this->input->post('submit')) {
					...
					$data['NomEtuAbsInfo'] = $this->show_model->get_nom_abs($idUser);
		...
		public function fill_ens() {
			...
			$data['fillensInfo'] = $this->edit_model->fill_admin($email); 
