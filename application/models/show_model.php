...
/* mine*/
	public function get_nom_abs($idUser) {

		$query = $this->db->query(
			"SELECT DISTINCT etudiant.nom_etu, etudiant.CNE
			FROM enseignant, affecter, module, etudier, etudiant, seance, absence
			WHERE affecter.id_user = enseignant.id_user
			AND affecter.id_module = module.id_module
			AND etudier.id_user = etudiant.id_user
	...
	public function get_coursInfo($idUser){

		$sql = "SELECT user.id_user,module.intitule_module,seance.date_seance,seance.heure_debut,seance.heure_fin,seance.type_seance 
		FROM module,etudier,seance,user 
		WHERE module.id_module = etudier.id_module 
		AND etudier.id_module = seance.id_module 
		AND etudier.id_user = ?";
